...
namespace ReFolder.Dir
{
    ...
    { 
        ...
        public void AddChildToChildrenList(IEditableDirWithChildrenAndParent child)
        {
            ...
            if (child == null) throw new ArgumentNullException();
            if (DirValidate.GetDefaultInstance().IsNameExistingInChildrenDirs(this, child.Description.Name)) throw new InvalidOperationException($"folder named as {child.Description.Name}  exist in structure");
            child.ParentDir = this;
            child.AutoGenerateDirFullName();

            if (DirValidate.GetDefaultInstance().IsfolderExisting(child.Description.FullName)) throw new InvalidOperationException("you can't create folder that exist");
            Children.Add(child);
        }
        // dodaje childrenDiry
